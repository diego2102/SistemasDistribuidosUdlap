...
import com.Innova4D.Interface.Patineta;
...
public class ControlTower extends UnicastRemoteObject implements RemoteInterface {
  ...
  /**
	...
	@Override
	public Boolean checkInPatineta(Patineta a) throws RemoteException {
		if(this.mapaPista[a.getX()][a.getY()] == null)
			this.mapaPista[a.getX()][a.getY()] = a;
		return true;
	}
	...
	@Override
	public Patineta getPatineta(String id, int c) throws RemoteException {
		Patineta a = null;
		for (int i = 0; i < 8; i++) {
			if(this.mapaPista[c][i] != null){
				a = (Patineta) this.mapaPista[c][i];
				if(!a.getId().equals(id))
					a = null;
			}
		}
	...
	@Override
	public Boolean moverPatineta(Patineta a, int c) throws RemoteException {
		Boolean flag = false;
		int newY = a.getY() + 1;
		if (newY < 8) {
			try {
